  ...
  return (
    <div className={`flex items-center gap-4 ${className}`}>
      <div className="relative group">
        <div className={`absolute -inset-1 rounded-full blur opacity-25 group-hover:opacity-50 transition duration-1000 ${light ? 'bg-white' : 'bg-brand-accent'}`}></div>
        <div className={`relative w-14 h-14 rounded-2xl flex items-center justify-center shadow-2xl transition-all duration-500 overflow-hidden ${light ? 'bg-white' : 'bg-brand-primary'}`}>
          <svg viewBox="0 0 100 100" className="w-10 h-10" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M50 15C30.67 15 15 30.67 15 50C15 69.33 30.67 85 50 85C69.33 85 85 69.33 85 50" stroke={light ? "#0f172a" : "#f59e0b"} strokeWidth="8" strokeLinecap="round" className="opacity-20" />
            ...
            <path d="M35 55L50 40L65 55V70H35V55Z" fill={light ? "#0f172a" : "#f59e0b"} />
            <circle cx="50" cy="40" r="4" fill={light ? "#f59e0b" : "#fff"} />
          </svg>
        </div>
      </div>
      <div className="flex flex-col">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-[10px] uppercase font-bold tracking-[0.25em] ${light ? 'text-brand-accent' : 'text-emerald-600'}`}>Efficiency</span>
